...
def create_csv_temp(file_path: Path) -> None:
    ...
    with open(file_path, "w", newline="", encoding="utf-8") as csvfile:
        writer = csv.DictWriter(
            csvfile, fieldnames=["name", "last_modified", "isVersionOf"]
        )
        writer.writeheader()
...
def write_to_csv(jsonld_file: Path, csv_file_path: Path) -> None:
    ...
    try:
        with open(jsonld_file, "r", encoding="utf-8") as f:
            ...
            if isinstance(data, list) and len(data) > 0:
                ...
                if (
                    modified_data
                    and isinstance(modified_data, list)
                    and len(modified_data) > 0
                ):
    ...
    with open(csv_file_path, "a", newline="", encoding="utf-8") as csvfile:
        ...
        row_data = {"name": name, "last_modified": last_modified, "isVersionOf": ror_id}
...
def create_ldes_fragment(
    csv_file_path: Path, current_version: str, next_version: str
) -> None:
    organizations = []
    ...
    vars = {
        ...
        "next_fragment_time": get_git_last_commit_date_for_release(next_version)
        or None,
        "retention_period": 1,
        "base_uri": "http://w3id.org/kgfixed/ror.org/",
        "data": {"qres": organizations},
    }
